...
def parse_input(file_path: str = "./day_6/small_input.txt") -> Race:
    ...
    return Race(time=int(times[0]), distance=int(distances[0]))
...
def binary_search(min_val, max_val, record_distance, race_time) -> int:
    ...
    if (max_val - min_val) == 1:
        if exceeds_record(
            record_distance=record_distance, race_time=race_time, charge_time=min_val
        ):
            return min_val
        else:
            return max_val
    ...
    if exceeds_record(
        record_distance=record_distance, race_time=race_time, charge_time=test_val
    ):
