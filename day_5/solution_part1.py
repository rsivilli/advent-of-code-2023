...
class GenericTransform:
    ...
    def get_lookup(self, destination_val):
        if (
            destination_val < self.destination_gte
            or destination_val > self.destination_lte
        ):
            raise ValueError(
                f"This is not a valid mapping for source {destination_val}. Must be between {self.destination_gte}-{self.destination_gte}"
            )
    ...
    def get_output(self, source_val):
        if source_val < self.source_gte or source_val > self.source_lte:
            raise ValueError(
                f"This is not a valid mapping for source {source_val}. Must be between {self.source_gte}-{self.source_lte}"
            )
...
class Link:
    ...
    def lookup(self, destination_val: int) -> int:
        for trans in self.destination_transforms:
            if (
                destination_val >= trans.destination_gte
                and destination_val <= trans.destination_lte
            ):
