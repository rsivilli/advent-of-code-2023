...
def merge_tracker(
    primary_tracker: dict[int, dict[int, list[int]]],
    update_to_tracker: dict[int, dict[int, list[int]]],
    ...
    for line in focus_lines:
        ...
        for gear_pos in (
            primary_tracker.get(line, {}).keys()
            | update_to_tracker.get(line, {}).keys()
        ):
            out[line][gear_pos] = primary_tracker.get(line, {}).get(gear_pos, [])
            out[line][gear_pos].extend(
                update_to_tracker.get(line, {}).get(gear_pos, [])
            )
    return out


def find_gearing_sums(
    model_line: LineObject,
    verifier_lines: list[LineObject],
    max_length: int,
    offset: int = 0,
):
    ...
    for model in model_line.model_number_matches:
        for line_number, line in enumerate(verifier_lines):
            tmp = line.find_verifier_collision(
                *get_match_bounding_box(model, max_length)
            )
            if tmp:
                gearing[line_number + offset] = gearing.get(line_number + offset, {})
                gearing[line_number + offset][tmp] = gearing[line_number + offset].get(
                    tmp, []
                )
    ...
    with open(file) as f:
        ...
        for line_number, line in enumerate(f):
            ...
            if len(line_buffer) > 3:
                line_buffer.pop(0)
            if len(line_buffer) == 3:
                tmp = find_gearing_sums(
                    line_buffer[1], line_buffer, max_line_length, line_number - 2
                )
                gear_tracker = merge_tracker(
                    gear_tracker,
                    tmp,
                    focus_lines=range(line_number, line_number - 3, -1),
                )
            # First line
        ...
        # last line
        tmp = find_gearing_sums(
            line_buffer[2], [line_buffer[1]], max_line_length, line_number - 1
        )
        gear_tracker = merge_tracker(
            gear_tracker, tmp, focus_lines=range(line_number, line_number - 2, -1)
        )
        print(gear_tracker)
        ...
        for line in gear_tracker.keys():
            for gear in gear_tracker[line].keys():
                if len(gear_tracker[line][gear]) == 2:
                    running_sum += (
                        gear_tracker[line][gear][0] * gear_tracker[line][gear][1]
                    )
        print(running_sum)
