...
INDICATOR_SEARCH_PATTERN = (
    r'(?P<model_indicator>[!"#\$%&\'\(\)\*\+,\-\/:\;<=>?@\[\]\^_`{\|}~])'
)
...
class LineObject:
    def __init__(
        ...
        indicator_search_patter=INDICATOR_SEARCH_PATTERN,
    ) -> None:
    ...
    def find_verifier_collision(
        self, lower_bound: int, upper_bound: int, arr=None
    ) -> int | None:
        ...
        if arr[pos] >= lower_bound and arr[pos] <= upper_bound:
            ...
            return self.find_verifier_collision(
                lower_bound, upper_bound, arr[pos + 1 :]
            )
...
def find_verified_models(
    model_line: LineObject, verifier_lines: list[LineObject], max_length: int
):
    verified_models = []
        ...
        for line_number, line in enumerate(f):
            ...
            if len(line_buffer) == 3:
                verified_models.extend(
                    find_verified_models(line_buffer[1], line_buffer, max_line_length)
                )
            elif len(line_buffer) == 2:
                verified_models.extend(
                    find_verified_models(line_buffer[0], line_buffer, max_line_length)
                )

                # evaluate first line
        verified_models.extend(
            find_verified_models(line_buffer[2], [line_buffer[1]], max_line_length)
        )
